...
Description: SEO Auto Linker inserts links on keywords automatically with optimized performance and security.
*/

if (!class_exists('SEOAutoLinks')) :
    class SEOAutoLinks {
        private $db_option = 'SEOAutoLinks';
        private $options;
    ...
    $text
            ...
            if (!empty($this->options['customkey']) || !empty($this->options['customkey_url'])) {
                ...
                foreach ($custom_keywords as $keyword => $url) {
                    ...
                    if (in_array($keyword_lower, $ignored_keywords, true)) continue;
                    if (trailingslashit($url) === $current_url) continue;
            ...
            if ($this->options['lposts'] || $this->options['lpages']) {
                ...
                foreach ($posts as $post_item) {
                    ...
                    if ($title_check === $current_title) continue;
                    if (in_array($title_check, $ignored_keywords, true)) continue;
            ...
            if ($this->options['lcats']) {
                ...
                foreach ($categories as $cat) {
                    ...
                    if (in_array($cat_check, $ignored_keywords, true)) continue;
            ...
            if ($this->options['ltags']) {
                ...
                foreach ($tags as $tag) {
                    ...
                    if (in_array($tag_check, $ignored_keywords, true)) continue;
...
foreach ($all_keywords as $item) {
    if ($max_links && $links_added >= $max_links) break;
    if ($max_single_url && ($url_counts[$item['url']] ?? 0) >= $max_single_url) continue;
    ...
    if ($strpos_func($text_normalized, $keyword_normalized) === false) continue;
...
return trim($text);
        ...
        private function get_cached_posts() {
            ...
            $posts = wp_cache_get($cache_key, 'wpa-seo-auto-linker');
            ...
            if (false === $posts) {
                ...
                wp_cache_set($cache_key, $posts, 'wpa-seo-auto-linker', 86400);
            }
        ...
        private function get_cached_categories($min_usage) {
            ...
            $categories = wp_cache_get($cache_key, 'wpa-seo-auto-linker');
            ...
            if (false === $categories) {
                ...
                wp_cache_set($cache_key, $categories, 'wpa-seo-auto-linker', 86400);
            }
        ...
        private function get_cached_tags($min_usage) {
            ...
            $tags = wp_cache_get($cache_key, 'wpa-seo-auto-linker');
            ...
            if (false === $tags) {
                ...
                wp_cache_set($cache_key, $tags, 'wpa-seo-auto-linker', 86400);
            }
        ...
        public function handle_options() {
            ...
            if (isset($_POST['submitted'])) {
                ...
                echo '<div class="updated"><p>' . esc_html__('Plugin settings saved.', 'wpa-seo-auto-linker') . '</p></div>';
            }
        ...
        public function enqueue_admin_scripts($hook) {
            if (strpos($hook, 'wpa-seo-auto-linker') === false) {
                return;
            }
            
            wp_enqueue_script('tagsjs', plugins_url('/js/load.js', __FILE__), ['jquery'], '2.0', true);
            wp_enqueue_style('tagscss', plugins_url('/css/wpa-seo-auto-linker-style.css', __FILE__), [], '2.0');
        ...
        public function delete_cache($id) {
            ...
            for ($i = 1; $i <= 20; $i++) {
                wp_cache_delete('seo-links-categories-' . $i, 'wpa-seo-auto-linker');
                wp_cache_delete('seo-links-tags-' . $i, 'wpa-seo-auto-linker');
            }
        }
    }
endif;
